...
for locations in antennas.values():
    for i in range(len(locations)):
        for j in range(i + 1, len(locations)):
            ...
            x = 0
            while True:
                new_r, new_c = r1 + delta_r * x, c1 + delta_c * x
                if 0 <= new_r < ROWS and 0 <= new_c < COLS:
                    if (new_r, new_c) not in unique_antinodes:
                        unique_antinodes.add((new_r, new_c))
                        num_antinodes += 1
                else:
                    break
                x += 1
            x = 0
            while True:
                new_r, new_c = r2 - delta_r * x, c2 - delta_c * x
                if 0 <= new_r < ROWS and 0 <= new_c < COLS and (new_r, new_c):
                    if (new_r, new_c) not in unique_antinodes:
                        unique_antinodes.add((new_r, new_c))
                        num_antinodes += 1
                else:
